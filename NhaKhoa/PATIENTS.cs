...
namespace NhaKhoa
{
    class PATIENTS
    {
        ...
        public bool InsertPatient(string fullName, string address, string phoneNumber, DateTime dateOfBirth, string gender, MemoryStream image)
        {
        ...
        {
            SqlCommand command = new SqlCommand("DELETE FROM Patients WHERE PatientID = @id", mydb.getConnection); command.Parameters.Add("@id", SqlDbType.Int).Value = id;
            mydb.openConnection();
            if ((command.ExecuteNonQuery() == 1))
            {
                mydb.closeConnection();
